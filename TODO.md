...
- [x] **Week 2** — `/v1/chat` SSE pass-through with Gemini primary + Groq fallback, dual-header auth, per-turn Axiom telemetry, quota gate, cost estimator, Gemini connection prewarm at boot.
- [x] **Week 2.5** (this session) — full end-to-end live smoke test against real Supabase + Gemini; PostgREST schema-exposure bug found and fixed (migration `20260520000000_expose_glassnote_schema.sql`); Gemini model names updated to current generation (`gemini-flash-latest`); two timestamp-format bugs in `/v1/trial/start` and `/status` caught by integration tests; **48-test suite** (30 unit + 18 integration) with `npm run test:watch` loop; GitHub Actions CI for typecheck + unit tests; client `glassnote://` protocol handler wired across all three Electron launch paths (open-url / cold argv / second-instance argv); PostHog server-side events wired into magic-link / trial / Lemon webhook routes; per-route rate limit (5 req/IP/hour) on `/auth/magic-link/request`; full BACKEND.md §13 security audit recorded in `glassnote-backend/SECURITY.md`. `/v1/embed` deferred to v2 (no current client caller). Pushed: BE `4f73bac`, client `f5ed6ea`.
- [ ] **Week 3** — Lemon Squeezy product + checkout URL swap on client (blocked on LS verification), Redis-backed circuit breaker (only if scaling to ≥2 Fly machines).
- [ ] **Week 4** — Apple notarization config, deploy to Fly + DNS cutover for `api.glassnote.site`.
